...
7. Convenience, speed, "we'll harden it before launch" — **never a tiebreaker**
...
**Uniformly** is the load-bearing word: two handlers doing it one way is a
coincidence; every shipped handler doing it one way is the project's
architecture, and this skill's default lost to it.

**`[NN]` rules are exempt from all of this.** Security and correctness
invariants never lose to a repo pattern — a project that uniformly omits authz
checks is uniformly wrong. This carve-out is for structure, never for safety.

What this does **not** license: skipping an `[ARCH]` rule because one handler
finds it inconvenient, or because the repo is merely *inconsistent* here.
