...
Single source of truth for a personal Codex **skill library**. Each skill is a folder
`skills/<name>/` containing a `SKILL.md` (plus optional `references/` and `scripts/`). The repo is
**symlinked into `~/.Codex/skills`** (via `install.sh`), so editing a `SKILL.md` here makes it live
in Codex immediately — the "update" and "backup" loops are the same `git commit`. There is no
...
node scripts/cli.mjs install              # install ALL skills into ~/.Codex/skills
...
- **`description:`** — a trigger-rich blurb (often listing literal phrases and slash-commands like
  `/sec.audit`, `/cq.init`). This is what Codex matches on to decide whether to load the skill, so
...
resolves in the repo layout AND in `~/.Codex/skills` (both symlink and copy installs place
...
- **Three passes, none seeing the others.** A fresh Claude reviewer subagent
  (A), a separate `codex` process (B), and the stack's code-quality specialist
  running its rule-by-rule **Verification Pass** (C) all read the same diff
  blind; reconciliation is the orchestrator's judgment and attributes each
  finding `[claude]`/`[codex]`/`[rules]` — the labels the skill actually emits.
  A and B are free-form and find what they notice; C exists because the worst
  defects are **absences** — its table has a row per rule in force, so an
  unperformed check surfaces as an empty box instead of as silence. C shares
  A's model, so it adds method diversity, not model diversity — never report it
  as a third opinion. Since there is no Design Contract here, C derives the
  rules in force from the diff (every `[NN]` rule + every rule whose surface the
  diff touches + a mandatory `AI-FM` row), and a FAIL is a finding rather than a
  gate. Dropping a finding requires a contract failure or a **named rule ID** —
  a bare "style-only" retires nothing.
...
- **`spec-driven`** — *bundles* `scripts/*.sh` in the repo (`specify.sh`, `plan.sh`, `tasks.sh`, …) and
  its `setup.sh` copies them into a target project's `.Codex/skills/spec-driven/scripts/` at first use.
...
- **`nn-guard`** — *generates* its script into the target project at install time, writing
  `.Codex/hooks/nn-guard.sh` (a `mv`/`chmod` in the `SKILL.md`), then wiring it as a PostToolUse hook
...
1. Create `skills/<name>/SKILL.md` (or edit an existing one) — with the symlink install, edits are
   already live in Codex, no re-install needed.
